...
try{
 for(const [width,height] of [[320,568],[375,667],[390,844],[430,932],[844,390],[768,1024],[1440,900]]){
  const context=await b.createBrowserContext();const p=await context.newPage();await returningUser(p);await p.setViewport({width,height,isMobile:width<1000,hasTouch:width<1000,deviceScaleFactor:1});await p.goto(process.env.QA_URL||'http://127.0.0.1:4173/',{waitUntil:'networkidle0'});await p.waitForSelector('.verse.active');const phone=width<=600||height<500;const rect=async s=>p.$eval(s,e=>{const r=e.getBoundingClientRect();return {x:r.x,y:r.y,w:r.width,h:r.height,b:r.bottom}})
